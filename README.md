...
4. **태그**: NOTE_INDEX.md의 "기존 태그" 목록에서 먼저 찾아 재사용하세요. 의미가 겹치는 태그가 있으면 새로 만들지 말고 기존 태그를 사용합니다. 꼭 필요한 경우에만 새 태그를 추가하세요. 기술명은 영문 소문자 (react, typescript, docker), 개념/도메인은 한글 가능
...
GitHub Actions가 이 파일을 읽고 자동으로 기존 노트 맨 아래 `## 자동 연결` 섹션에
`[[source_slug]] — *source_slug 노트 작성 중에 추가된 연결입니다.*` 형식으로 연결을 추가한 뒤 파일을 삭제합니다.
